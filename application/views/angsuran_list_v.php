		...
		$.ajax({
			...
			success	: function(result){
				...
				if((result.sisa_ags == 0) || (result.total_tagihan <= 0)) {
					...
					let getTempo = "<?=$row_pinjam->tempo?>"
					let tempo = moment(getTempo, 'YYYY-MM-DD HH:mm:ss')
					let curentDate = moment().format('YYYY-MM-DD HH:mm:ss')

					let durasi = moment.duration(tempo.diff(curentDate))
					let selisih = durasi.asDays() > 0 ? 0 : Math.abs(Math.ceil(durasi.asDays())) 
