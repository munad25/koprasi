...
class Pinjaman_m extends CI_Model {
	...
	function pengajuan_aksi() {
		...
		$this->db->where('harga', $result->nominal);
		$getBarang = $this->db->get();
		$barangResult = $getBarang->result()[0];

		$insert_arr = [
			'tgl_pinjam' => $result->tgl_input,
			'anggota_id' => $result->anggota_id,
			'barang_id' => $barangResult->id,
			'lama_angsuran' => $result->lama_ags,
			'jumlah' => $barangResult->harga,
			'bunga' =>  '2.00',
			'biaya_adm' => '1500',
			'lunas' => 'Belum',
			'dk' =>'K',
			'kas_id' => 1,
			'jns_trans' => 7,
			'update_data' => '0000-00-00 00:00:00',
			'user_name' => 'admin',
			'contoh' => 0
		];

		if($result->status == 1) {
			$this->db->insert('tbl_pinjaman_h', $insert_arr);
		}
